...
export default function GenerateInstructionModal({ onClose }) {
  ...
  const isGenerating = useSelector(
    (state) => state.instructions.isGenerating
  );

  const generateError = useSelector(
    (state) => state.instructions.generateError
  );
  ...
  async function handleSubmit(e) {
    ...
    const result = await dispatch(
      generateInstruction(profession.trim())
    );
    ...
    <div className={styles.overlay} onClick={onClose}>
      ...
      >
        ...
        <p className={styles.hint}>
          Введите профессию, для которой необходимо создать
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <button
            ...
            disabled={isGenerating || !profession.trim()}
          >
            {isGenerating
              ? "Генерация..."
              : "Создать инструкцию"}
